...
glm::vec3 PosIni(-154.0f, -36.0f, -239.0f);
bool active;
...
//mover carro
float posX_carro = 40;		//Variable para PosicionX
float posY_carro = 38;		//Variable para PosicionY
float posZ_carro = 56;		//Variable para PosicionZ
...
int main()
{
	...
	PlaySound("cancionSimpson.wav", NULL, SND_ASYNC | SND_LOOP);
	...
	Model Homero((char*)"Models/casa/homero.obj");
	Model Garage((char*)"Models/casa/p_garage.obj");
	Model Puerta((char*)"Models/casa/p_principal.obj");
	Model Carro((char*)"Models/casa/carro.obj");
	...
	stbi_set_flip_vertically_on_load(true);				//flipeo
	unsigned char *image;
	// Diffuse map
	image = stbi_load("images/fuego.jpg", &textureWidth, &textureHeight, &nrChannels, 0);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		view = camera.GetViewMatrix();
		model = glm::translate(tmp, glm::vec3(4, 38, 30));
		model = glm::translate(model, glm::vec3(posX, posY, posZ));
		model = glm::scale(model, glm::vec3(0.1f, 0.1f, 0.1f));
		model = glm::rotate(model, glm::radians(rot), glm::vec3(0.0f, 1.0f, 0.0));
		...
		view = camera.GetViewMatrix();
		model = glm::translate(tmp, glm::vec3(posX_carro, 38, posZ_carro));
		model = glm::translate(model, glm::vec3(posX, posY, posZ));
		...
		view = camera.GetViewMatrix();
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-28, 19, 33));
		model = glm::translate(model, glm::vec3(posX, posY, posZ));
		model = glm::scale(model, glm::vec3(0.1f, 0.1f, 0.1f));
		...
		view = camera.GetViewMatrix();
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(63, 0, -27));
		model = glm::translate(model, glm::vec3(posX, posY, posZ));
		model = glm::scale(model, glm::vec3(0.1f, 0.1f, 0.1f));
		model = glm::rotate(model, glm::radians(rot), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, glm::radians(-rotBraIzq), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		Homero.Draw(lightingShader);
		...
		glBindTexture(GL_TEXTURE_2D, texture1);
			...
			model = glm::mat4(1);
			model = glm::translate(model, glm::vec3(-180, -26, -232));
			model = glm::scale(model, glm::vec3(3.0f, 9.0f, 12.0f)); // Make it a smaller cube
			glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void DoMovement()
{
	...
	{
		posZ_carro += 1;
		//printf("x= %f y= %f z=%f", posX_carro, posY_carro, posZ_carro);
	}
	...
	{
		vector<const GLchar*> faces;
		faces.push_back("SkyBox/backD.tga");
		faces.push_back("SkyBox/bottomD.tga");
		faces.push_back("SkyBox/frontD.tga");
		faces.push_back("SkyBox/leftD.tga");
		faces.push_back("SkyBox/rightD.tga");
		faces.push_back("SkyBox/topD.tga");
		GLuint cubemapTexture = TextureLoading::LoadCubemap(faces);
	}
	if (keys[GLFW_KEY_V])
	{
